...
export class MemberController {
  ...
  @Post()
  @HttpCode(201)
  @ApiOperation({ description: 'Body: firstName, lastName, email, phone, address, dateOfBirth, isActive, maxBorrowLimit\nไม่ต้องใส่ header role' })
  ...
  @Get('search')
  @HttpCode(200)
  @ApiOperation({ description: 'Query param: query = ชื่อ (firstName หรือ lastName, partial match) หรือ ID เป็นตัวเลข\nHeader: role=admin\nถ้าไม่เจอจะได้รับ message: No data found' })
  ...
  @Patch(':query')
  @ApiOperation({ description: 'Path param: :query = ID หรือชื่อสมาชิก (exact match) เช่น \"2\" หรือ \"John Doe\"\nHeader: role=admin\nBody: เฉพาะ field ที่ต้องการแก้ เช่น { maxBorrowLimit: 5 }' })
  @ApiHeader({ name: 'role', description: 'admin', required: true })
  update(
  ...
  @ApiOperation({ description: 'Path param: :id = ID สมาชิกที่จะยืม, :bookId = ID หนังสือที่จะยืม\nHeader: role=member หรือ role=admin\nguest ยืมไม่ได้' })
  ...
  @ApiOperation({ description: 'Path param: :id = ID สมาชิกที่จะคืน, :bookId = ID หนังสือที่จะคืน\nHeader: role=member หรือ role=admin\nguest คืนไม่ได้' })
  @ApiHeader({ name: 'role', description: 'member หรือ admin', required: true })
  returnBook(
    @Param('id') id: string,
    @Param('bookId') bookId: string,
